...
/*****************************************************************************/
#define CAPTION    "animation_setcontext"
...
MY_CONTEXT g_my_context = { "red color", -1 };
...
static void draw_frame (HWND hWnd)
{
    ...
    /* draw the text */
    sprintf (str, "CTXT: name(%s) value(%d)", g_my_context.name, g_my_context.value);
